...
function addGroupOfItems(doc, startY, groupName, items) {
  doc.font("Helvetica-Bold").fontSize(12).text(groupName, 50, startY);
  ...
  items.forEach((item) => {
    ...
    doc
      ...
      .text(item.qty ? item.qty.toString() : "N/A", 450, currentY, {
        width: 100,
        align: "right",
      });
    currentY += 20;
...
function addGroupOfFItems(doc, startY, groupName, items) {
  ...
  items.forEach((item) => {
    doc
      ...
      .text(item.qty ? item.qty.toString() : "N/A", col4X, currentY, {
        width: 80,
        align: "right",
      })
      .text(item.weight ? item.weight.toString() : "N/A", col5X, currentY, {
        width: 80,
        align: "right",
      });
...
async function generatePDF(order_no, data, res) {
  ...
  if (orderDetails) {
    doc.text(`Ticket number: ${orderDetails.ticket_number || "N/A"}`, {
      align: "right",
    });
  } else {
  ...
  doc.image(qrCodeData, qrCodeX, currentY + 15, {
    width: qrCodeWidth,
    height: qrCodeHeight,
  });
